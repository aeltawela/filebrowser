...
Pasting a video URL checks the actual formats before populating choices. The dialog selects **Recommended**: a 4K source version when present, otherwise the highest reported source resolution. Within a resolution it favors SDR, smooth motion and broadly supported codecs. It preserves original streams without upscaling or recompression. This is a predictable selection policy, not a perceptual quality measurement.

The main list has one choice per available resolution. **Advanced formats** expands a separate selector for alternate codecs, HDR and frame rates. Each selection explains picture, sound, estimated size and playback, with codec details collapsed. Downloadable formats with unreported dimensions or codecs remain selectable as the original source, without invented resolution or audio details. An unavailable source leaves no invented quality choices. The strict 4K server fallback remains in place for older clients and requests without an explicit quality.

**MKV (Recommended)** is the default video container for preserving selected streams; MP4 is available, with playback still dependent on the chosen codecs. Audio-only downloads retain their source audio format. Audio language appears only for labeled source tracks. Subtitle choices include authored and native automatically generated captions, excluding synthetic translation lists. Changing the URL clears all source-specific selections immediately. Direct file downloads retain the original file.
