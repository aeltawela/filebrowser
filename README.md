...
Pasting a video URL checks the actual formats before populating choices. The dialog selects **Recommended**: a source version targeting 4K, or the highest available resolution below 4K. When only higher resolutions exist, it selects the smallest available. Within a resolution it favors SDR, smooth motion and broadly supported codecs. It preserves original streams without upscaling or recompression. This is a predictable selection policy, not a perceptual quality measurement.

The main list has one choice per available resolution. **Show more options** reveals alternate codecs, HDR and frame rates. Each selection explains picture, sound, estimated size and playback, with codec details collapsed. An unavailable source leaves no invented quality choices. The strict 4K server fallback remains in place for older clients and requests without an explicit quality.
